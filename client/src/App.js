...
function App() {
  ...
  const handleLogin = (userObj) => {
    ...
    setTimeout(() => {
      setUser({ ...userObj, role: userObj.role || 'Customer', username: userObj.username });
    }, 800);
  };
  ...
  return (
    <BrowserRouter>
      {(!user) ? (
        ...
        <Routes>
          <Route path="/customer" element={<CustomerDashboard {...dashboardProps} username={user.username} /> }>
          ...
          <Route path="*" element={<CustomerDashboard {...dashboardProps} username={user.username} /> } />
        </Routes>
      ) : user.role === 'Fisherman' ? (
        <FishermanDashboard {...dashboardProps} user={user} catches={catches} setCatches={setCatches} username={user.username} />
      ) : user.role === 'Inspector' ? (
        <InspectorDashboard {...dashboardProps} username={user.username} />
      ) : (
